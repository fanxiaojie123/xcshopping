...
public interface MessageSolrMapper {
   // select p.pid,p.catalog_name,p.price,p.picture,s.description,s.type,st.`name` from product p LEFT JOIN sku s ON p.description_id=s.sku_id LEFT JOIN skutwo st ON p.name_id=st.sku_nameid
    @Select("select m.m_id,m.m_name,m.m_picture,m.m_particulars,m.m_count,m.m_price,m.m_points,sc.search_category_name,s.s_name,s.s_address,s.s_phone,s.s_describe,s.s_picture from message m LEFT JOIN seller s ON m.s_id = s.s_id LEFT JOIN search_category sc ON m.search_category_id = sc.search_category_id where m_status=1")
    @Results({
